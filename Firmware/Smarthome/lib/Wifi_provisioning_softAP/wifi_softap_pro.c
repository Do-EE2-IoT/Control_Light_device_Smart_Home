...
extern uint16_t backup_address_for_timeout;
extern int backup_control_value_for_timeout;


void mqtt_callback(char *data, int data_size){
   ...
   cJSON *value_json = cJSON_GetObjectItemCaseSensitive(json, "value");
    ...
    ESP_LOGI(TAG, " Start send command ");
    client_send_set_msg_to_server(value,address_u16);
    backup_address_for_timeout = address_u16;
    backup_control_value_for_timeout = value;
    // backup = address_u16;
    // backup_control_value = value;

    //ble_mesh_collect_sensor_data(address_u16);
    // Cleanup
    free(address_s);
    free(value_s);
    cJSON_Delete(json);
    ...
    if (event_base == WIFI_PROV_EVENT) {
        ...
        switch (event_id) {
            ...
            case WIFI_EVENT_AP_STADISCONNECTED:
                ...
                output_set_level(GPIO_NUM_4,0);
                break;
            case WIFI_EVENT_STA_CONNECTED:
                 ESP_LOGI(TAG, "connected WIFI");
                 MQTT_client_init();
                 mqtt_client_callback_register(mqtt_callback);
                 output_set_level(GPIO_NUM_4,1);
                 break;
#endif
            default:
                break;
...
void wifi_provisioning_softap_init(void)
{
