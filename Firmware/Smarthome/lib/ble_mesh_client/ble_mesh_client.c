/**
 * @brief: This is a library for Bluetooth low enery mesh model server generic on off
...
#define TAG "SWITCH_GATEWAY"
#define CID_ESP 0x02E5
#define BLE_MESH_DEVICE_NAME "GATEWAY"

// For publish purpose
uint16_t address;
int value;

uint16_t backup_address_for_timeout;
int backup_control_value_for_timeout;
                                               ...
                                               esp_ble_mesh_generic_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_GENERIC_CLIENT_SET_STATE_EVT:
        ...
        if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET) {
            ESP_LOGI(TAG, "ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET, onoff %d, address %04x" PRIx32, param->status_cb.onoff_status.present_onoff, param->params->ctx.addr);
            cJSON *json_obj = cJSON_CreateObject();
            if (json_obj == NULL) {
                ESP_LOGE(TAG, "Cannot create JSON");
                return;
            }
             // Chuyển đổi địa chỉ từ số nguyên thành chuỗi hex
            address = param->params->ctx.addr;
            value = param->status_cb.onoff_status.present_onoff;
            char address_str[8];
            sprintf(address_str, "0x%04X", address);

             // Thêm các trường vào đối tượng JSON
            cJSON_AddStringToObject(json_obj, "address", address_str);
            cJSON_AddNumberToObject(json_obj, "value", value);

             // Chuyển đối tượng JSON thành chuỗi JSON
            char *json_str = cJSON_Print(json_obj);
            if (json_str == NULL) {
               ESP_LOGI(TAG,"Không thể chuyển đổi thành chuỗi JSON");
               cJSON_Delete(json_obj);
            return ;
            }
            mqtt_client_publish("/device/sub/ID=0002",1,json_str);
    ...
    case ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT:
        ESP_LOGI(TAG, "ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT - LIGHT onoff %d",param->status_cb.onoff_status.present_onoff);
        break;
    case ESP_BLE_MESH_GENERIC_CLIENT_TIMEOUT_EVT:
        ESP_LOGI(TAG, "ESP_BLE_MESH_GENERIC_CLIENT_TIMEOUT_EVT");
        if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET) {
            vTaskDelay(1000/portTICK_PERIOD_MS);
            client_send_set_msg_to_server(backup_control_value_for_timeout,backup_address_for_timeout);
            ESP_LOGI(TAG, "Control backup %d", backup_control_value_for_timeout);
...
/**
 * @brief This is function use to client send set massage for server
 * @param Control_Value - State of light want to control
 * @param Des_addr : Destination address ( LIGHT )
*/
...
/**
 * @brief This is function use to client send get massage for server
 * @param Des_addr : Destination address ( LIGHT )
*/
void client_send_get_msg_to_server(uint16_t Des_address){
    esp_ble_mesh_client_common_param_t common = {0};
    esp_ble_mesh_generic_client_get_state_t get = {0}; // No payload for Generic OnOff Get
    esp_err_t err = ESP_OK;
    ...
    common.ctx.app_idx = store.app_idx; 
    /* Address sending */
    common.ctx.addr = Des_address; // Replace with the server's unicast address
    common.ctx.send_ttl = 3;
...
/**
 * @brief Initalize BLE MESH function for client - switch
*/
