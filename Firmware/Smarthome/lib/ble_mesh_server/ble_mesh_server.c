...
static esp_ble_mesh_gen_onoff_srv_t onoff_server_0 = {
    // .rsp_ctrl.get_auto_rsp = ESP_BLE_MESH_SERVER_AUTO_RSP, -> event auto equal to zero ?
                                               ...
                                               esp_ble_mesh_generic_server_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_GENERIC_SERVER_RECV_SET_MSG_EVT:
        ...
        if (param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET ||
            ...
            output_set_level(GPIO_NUM_2,param->value.set.onoff.onoff);
            if(param->ctx.recv_dst == element_address_1_u16){
                output_set_level(GPIO_NUM_17, param->value.set.onoff.onoff);
                ESP_LOGI(TAG, "Button 1 change state!");
            }
            else if(param->ctx.recv_dst == element_address_2_u16){
                output_set_level(GPIO_NUM_22, param->value.set.onoff.onoff);
